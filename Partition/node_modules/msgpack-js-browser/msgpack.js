...
function encode(value, view, offset) {
  ...
  if (type === "number") {
    // Floating Point
    /** Floating point number would take 1 byte for identification,
        8 bytes for representation*/
    if ((value << 0) !== value) {
  ...
  if (type === "object") {
    ...
    var size;
    /** Add the identifier for the type of the object
...
/** Get the number of bytes to be allocated to the View.
JavaScript handles binary using Views*/
function sizeof(value) {
  ...
  if (type === "number") {
    // Floating Point
    // double
    /** A Floating point would take 9 bytes:
        1 byte for the identification of DS, 8 bytes for signed 64 bit representation*/
    if (value << 0 !== value) return 9;

    // Integers
    /** An Integer can take varying number of bytes, depending on its size.*/
    if (value >=0) {
      // positive fixnum
      /** Fix number less than 128. Can be represented in 1 byte*/
      if (value < 0x80) return 1;
      // uint 8
      /** Number between 128 and 256 would take 2 bytes:
          1 byte for identification of DS, 1 byte for unsigned 8 bit representation*/
    ...
    /**Negative numbers would be handled the same way as positive, except they would be
  ...
  if (type === "object") {
    ...
    /**Add the number of bytes the length of the object would take to the size of the object*/
    if (length < 0x10) {
      return 1 + size;
    }
    if (length < 0x10000) {
